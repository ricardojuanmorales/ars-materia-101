# Ars Materia 101 — Mapa de Ruta Maestro F0–F10

**Versión:** 1.1  
**Estado:** ACTIVO  
**Fecha:** 2026-09-08  
**Función:** carta de navegación estratégica; no sustituye Estado Maestro, Gates ni bitácoras.  
**Fuente de verdad:** GitHub `main`

## 1. Propósito

Mantener una vista compacta y recuperable del proyecto completo para que producción educativa, documentación metodológica, trazabilidad y evolución del protocolo avancen como un solo sistema.

Debe permitir responder rápidamente:
- dónde estamos;
- qué está aprobado;
- qué puede producirse ahora;
- qué debe esperar;
- cuál es el próximo Gate;
- qué evidencia de proceso debe conservarse;
- cómo una decisión local afecta el protocolo transferible.

## 2. Principios rectores

1. Framework Genérico V4.1.0 activo de forma integral y proporcional.
2. `integrar antes que añadir`.
3. GitHub `main` = fuente versionada de verdad.
4. un producto vivo por función + Git como genealogía.
5. estructura completa, activación según necesidad.
6. función antes que formato para ubicar documentos.
7. trazabilidad metodológica y producción educativa tienen igual importancia operativa.
8. Humano decide propósito, cambios sustantivos y Gates.
9. IA analiza, integra, propone y reduce complejidad.
10. Máquina ejecuta, verifica estado y conserva trazabilidad.
11. DUA, accesibilidad, ética y sostenibilidad se anticipan.
12. no aumentar granularidad sin una función nueva demostrable.

## 3. Vista F0–F10

| Tramo | Función en Ars Materia | Estado práctico |
|---|---|---|
| **F0** | autoridad, repositorio, estado, continuidad y gobernanza | BASE OPERATIVA ESTABLECIDA |
| **F1** | fundamentos histórico-epistemológicos y arquitectura conceptual | HERENCIA RECTORA DISPONIBLE |
| **F2** | alineación pedagógica, evidencia, evaluación, DUA y apoyos | HERENCIA PARCIAL APROBADA DEL PROTOTIPO |
| **F3** | trayectoria FEHE H1-H10, PROEMA y arquitectura longitudinal | RECTOR APROBADO; PROFUNDIZACIÓN EN CURSO |
| **F4** | microdiseño/OVAt | NO ABRIR PARA H1 ANTES DE GATE FEHE-H1 |
| **F5** | prototipado integrado | PENDIENTE |
| **F6** | DUA + IA + tecnología + lúdica operacional | PENDIENTE / TRANSVERSAL ANTICIPADO |
| **F7** | auditoría integral de calidad | PENDIENTE |
| **F8** | producción y montaje Moodle/LMS | PENDIENTE |
| **F9** | validación con evidencia real | PENDIENTE |
| **F10** | metaevaluación, transferencia, archivo y evolución | PENDIENTE |

## 4. Carril de producción actual

```text
Documento Rector aprobado
→ FEHE-H1 profunda CANDIDATA
→ Gate humano FEHE-H1
→ especificación y producción OVAt-H1
→ preparación/montaje Moodle
→ QA + auditoría inversa
→ Gate integral H1
→ extracción del patrón FEHE transferible
→ evaluación humana para posible propagación H2-H10
```

No se reinicia artificialmente el proyecto desde F0. Las funciones F0-F3 se mantienen como gobernanza y herencias activas mientras el piloto avanza de forma express.

**Regla vigente del piloto:** aprobar FEHE-H1 no convierte todavía H1 en patrón ni autoriza propagación H2-H10. Primero debe comprobarse H1 como sistema integral.

## 5. Carril metodológico y de trazabilidad

Cada avance sustantivo debe dejar, de forma proporcional:

```text
decisión / hipótesis
→ evidencia o fuente
→ artefacto producido
→ Gate o estado
→ aprendizaje metodológico
→ impacto en protocolo transferible, si existe
```

Materia Mundi funciona como evidencia histórica y referencia de implementación avanzada. Ars Materia prueba transferencia y adaptación sin copiar mecánicamente decisiones curriculares.

## 6. Umbral de documentación

Documentar cuando cambie al menos uno de estos elementos:
- autoridad;
- estado o Gate;
- arquitectura;
- metodología reusable;
- ruta canónica;
- criterio de aceptación;
- riesgo material;
- aprendizaje transferible al protocolo.

No documentar por ceremonia detalles reversibles sin efecto sistémico.

## 7. Próximo Gate

**FEHE-H1 profunda candidata → validación humana → aprobación, aprobación condicionada o revisión.**

Un GO FEHE-H1 autoriza abrir el piloto OVAt-H1; no autoriza todavía extraer patrón ni propagar H2-H10. Esa posibilidad sólo se evalúa después del Gate integral H1.
